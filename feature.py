...
class ColorGradientPyramid(object):
    ...
    def _update(self):
        ...
        if len(self.src.shape) == 2 or \
            ...
            sobel_3dx = sobel_3dx.astype(np.float32)  # data type must to be converted
            sobel_3dy = sobel_3dy.astype(np.float32)
            ...
            sobel_dx = sobel_3dx[xx.T, yy.T, inds]
            # sobel_dx_vis = cv2.convertScaleAbs(sobel_dx)
            sobel_dy = sobel_3dy[xx.T, yy.T, inds]

        sobel_ag = cv2.phase(sobel_dx.astype(np.float32),
                             sobel_dy.astype(np.float32), angleInDegrees=True)
        self.angle = hysteresisGradient(
            magnitude.astype(np.float32), sobel_ag, self.weak_thresh**2)
    ...
    def extract_template(self) -> Template:
            ...
            for j in range(nms_ksize, w-nms_ksize, 1):
                if local_mask[i, j] and magnitude_valid[i, j]:
                    score = self.magnitude[
                        i-nms_ksize:i + nms_ksize, j-nms_ksize:j+nms_ksize].max()
                    if score == self.magnitude[i, j]:
                        magnitude_valid[
                            i-nms_ksize:i + nms_ksize, j-nms_ksize:j+nms_ksize] = 0
                        magnitude_valid[i, j] = 1
                        if score > threshold_sq and self.angle[i, j] > 0:
                            label = -1 if self.angle[i, j] > 128 else int(
                                    np.log2(self.angle[i, j]))
        ...
        templ.features = self._select_scattered_features(
            candidates, self.num_feat, distance)
